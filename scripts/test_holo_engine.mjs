...
function mockContext(options = {}) {
  const noop = () => {};
  let textureId = 0;
  const metrics = { mipmaps: 0, repeatParameters: 0, deletedTextures: 0, deletedPrograms: 0, draws: 0, viewports: 0, uniforms: new Map() };
  const context = {
    ...
    generateMipmap: () => { metrics.mipmaps += 1; }, getError: () => 0, getParameter: () => 4096, viewport: () => { metrics.viewports += 1; },
    ...
    drawArrays: () => { metrics.draws += 1; }, deleteTexture: () => { metrics.deletedTextures += 1; }, deleteBuffer: noop, deleteVertexArray: noop,
  };
...
const drawsBeforeExternalFrame = secondCanvas.context.metrics.draws;
secondRenderer.renderPointerFrame(0.68, -0.24, 120);
assert.equal(secondCanvas.context.metrics.draws, drawsBeforeExternalFrame + 1, "an external pointer frame renders once");
assert.equal(secondRenderer.diagnostics().mode, "external");
assert.equal(secondRenderer.diagnostics().animationPending, false, "external pointer frames cancel the internal RAF");
const viewportsBeforeRepeatedResize = secondCanvas.context.metrics.viewports;
const drawsBeforeRepeatedResize = secondCanvas.context.metrics.draws;
secondRenderer.resize();
secondRenderer.resize();
assert.equal(secondCanvas.context.metrics.viewports, viewportsBeforeRepeatedResize, "same-size resizes skip viewport updates");
assert.equal(secondCanvas.context.metrics.draws, drawsBeforeRepeatedResize, "same-size resizes skip redraws");
